...
class potential_member_selector extends \user_selector_base {
    ...
    public function find_users($search) {
        ...
        $sql .=" LEFT JOIN {" . constants::M_MEMBERTABLE .  "} m ON (m.memberuserid = u.id AND m.klassid = $this->klassid)";
        $sql .=" WHERE m.id IS NULL AND $wherecondition";
        ...
        //TODO: add to sql so teacher can only make groups from students visible via their groups
        ...
        if ($search) {
            $groupname = get_string('potusersmatching', constants::M_COMP, $search);
        } else {
            $groupname = get_string('potusers', constants::M_COMP);
        }
    ...
    protected function get_options() {
        ...
        $options['file'] = 'blocks/readaloudteacher/classes/potential_member_selector.php';
        return $options;
    }
}
